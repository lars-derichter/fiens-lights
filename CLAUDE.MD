...
**Purpose:** Control WS2812B LED strip with 7 different effects, adjustable via 3 potentiometers and 1 button.

## Architecture

### Hardware Interface
- **NeoPixel Strip**: Pin 6, 12 LEDs (configurable via `LED_COUNT`)
...
- **Potentiometers** (all 10kΩ):
  ...
  - A2: Speed (0-1023 → 10-100ms delay)
...
│   └── readSpeedFromPot() → uint8_t
├── Effect Functions (lines 90-426)
│   ├── effectOff()
│   ├── whiteLight()
│   ├── solidHue()
│   ├── pulseHue()
...
1. **Increment NUM_EFFECTS** (line 17)
   ```cpp
   #define NUM_EFFECTS 8  // was 7
   ```

2. **Create effect function** (after line 426)
   ```cpp
   void myNewEffect()
   {
     // Read pots
...
4. **Add effect name** (around line 177)
   ```cpp
   case 7:
...
// Faster effects:
uint8_t speed = map(filtered, 0, 1023, 5, 50);

// Slower effects:
uint8_t speed = map(filtered, 0, 1023, 50, 200);
```
...
- **Loop frequency:** ~100Hz when effects use 10ms delay
...
- **Memory usage:** ~40% SRAM, ~30% Flash (with 7 effects)
